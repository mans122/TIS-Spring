...
(function($) {
	...
	if ($('#sidebar-instagram-feed').length) {
		var userFeed = new Instafeed({
			...
			limit: 6,
			template:
				'<li><a href="{{link}}" target="_new"> <img src="{{image}}" /><ul class="likes-comments"><li><i class="fa fa-heart-o"></i><span>{{likes}}</span></li><li><i class="fa fa-comments-o"></i><span>{{comments}}</span></li></ul></a></li>'
		});
		userFeed.run();
	}
